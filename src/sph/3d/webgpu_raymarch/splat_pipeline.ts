 ...
 *
 * The resulting density texture is consumed by the raymarch renderer.
 *
...
export class SplatPipeline {
  ...
  constructor(device: GPUDevice) {
    ...
    //              minBounds(vec3<f32>), pad0(f32), maxBounds(vec3<f32>), pad1(f32),
    //              volumeSize(vec3<u32>), pad2(u32) = 64 bytes
   ...
   */
  recreate(config: RaymarchConfig, predictedBuffer: GPUBuffer): void {
