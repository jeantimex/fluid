...
export interface ApicGuiCallbacks {
...
export interface ApicGuiHandle {
  gui: GUI;
  destroy: () => void;
...
export function setupGui(
  ...
  initialGravity: Vec2,
  callbacks: ApicGuiCallbacks
): ApicGuiHandle {
  const state: ApicGuiState = {
  ...
  const gui = new GUI({ title: 'APIC Controls' });
  gui.close();
