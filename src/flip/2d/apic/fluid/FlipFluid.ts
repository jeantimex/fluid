...
// ─── FlipFluid ────────────────────────────────────────────────────────────────
//
// Implements a 2-D APIC (Affine Particle-In-Cell) fluid on a MAC (marker-and-
// cell) staggered grid.  The core idea is a two-way coupling between a
// Lagrangian particle layer and an Eulerian velocity grid:
//
//   1. Particles carry position, velocity, colour, and a local affine velocity
//      matrix.
//   2. Every step, particle affine velocity fields are splat onto the grid (P→G).
//   3. The grid solves for pressure to enforce incompressibility (∇·u = 0).
//   4. Grid velocities are transferred back to particles (G→P), reconstructing
//      both particle velocity and its local affine matrix.
//
...
export class FlipFluid {
  ...
  // Per-particle 2×2 affine velocity matrices, row-major:
  // [c00,c01,c10,c11] maps a local offset (dx,dy) to velocity delta.
  particleAffine: Float32Array;
  ...
  constructor(
    ...
    this.particleAffine = new Float32Array(4 * this.maxParticles);
  ...
  integrateParticles(
    ...
    for (let i = 0; i < this.numParticles; i++) {
      ...
      this.particleAffine[4 * i]     *= damping;
      this.particleAffine[4 * i + 1] *= damping;
      this.particleAffine[4 * i + 2] *= damping;
      this.particleAffine[4 * i + 3] *= damping;
  ...
  handleParticleCollisions(): void {
    ...
    for (let i = 0; i < this.numParticles; i++) {
      ...
      if (x < minX) { x = minX; this.particleVel[2 * i]     = 0.0; this.clearAffineRow(i, 0); }
      if (x > maxX) { x = maxX; this.particleVel[2 * i]     = 0.0; this.clearAffineRow(i, 0); }
      if (y < minY) { y = minY; this.particleVel[2 * i + 1] = 0.0; this.clearAffineRow(i, 1); }
      if (y > maxY) { y = maxY; this.particleVel[2 * i + 1] = 0.0; this.clearAffineRow(i, 1); }
  ...
  private clearAffineRow(particleId: number, row: 0 | 1): void {
    const offset = 4 * particleId + 2 * row;
    this.particleAffine[offset] = 0.0;
    this.particleAffine[offset + 1] = 0.0;
  }
  ...
  //   Splat each particle's local affine velocity field onto the MAC grid:
  //     v_face = v_particle + C_particle * (x_face - x_particle)
  //   This carries local rotational/shearing motion to the grid instead of only
  //   a constant particle velocity.
  //
  // toGrid = false (G→P, "grid-to-particle"):
  //   Interpolate post-projection grid velocities back to particles and fit a
  //   local affine matrix from the same neighbouring face samples. This is the
  //   APIC state used by the next P→G transfer.
  transferVelocities(toGrid: boolean): void {
    const n  = this.fNumY;
    const h  = this.h;
    const h1 = this.fInvSpacing;
    const h2 = 0.5 * h;
    ...
    for (let component = 0; component < 2; component++) {
      ...
      for (let i = 0; i < this.numParticles; i++) {
        ...
        if (toGrid) {
          const faceX0 = x0 * h + dx;
          const faceY0 = y0 * h + dy;
          const faceX1 = x1 * h + dx;
          const faceY1 = y1 * h + dy;
          const c0 = this.particleAffine[4 * i + 2 * component];
          const c1 = this.particleAffine[4 * i + 2 * component + 1];
          const pv = this.particleVel[2 * i + component];

          const pv0 = pv + c0 * (faceX0 - x) + c1 * (faceY0 - y);
          const pv1 = pv + c0 * (faceX1 - x) + c1 * (faceY0 - y);
          const pv2 = pv + c0 * (faceX1 - x) + c1 * (faceY1 - y);
          const pv3 = pv + c0 * (faceX0 - x) + c1 * (faceY1 - y);

          f[nr0] += pv0 * d0;  d[nr0] += d0;
          f[nr1] += pv1 * d1;  d[nr1] += d1;
          f[nr2] += pv2 * d2;  d[nr2] += d2;
          f[nr3] += pv3 * d3;  d[nr3] += d3;
        } else {
          ...
          if (totalValid > 0.0) {
            ...
            const value3 = f[nr3];
            const apicV =
              (w0 * value0 + w1 * value1 + w2 * value2 + w3 * value3) /
              totalValid;
            ...
            const dv0 = value0 - apicV;
            const dv1 = value1 - apicV;
            const dv2 = value2 - apicV;
            const dv3 = value3 - apicV;
            const m00 = w0 * rx0 * rx0 + w1 * rx1 * rx1 + w2 * rx2 * rx2 + w3 * rx3 * rx3;
            const m01 = w0 * rx0 * ry0 + w1 * rx1 * ry1 + w2 * rx2 * ry2 + w3 * rx3 * ry3;
            const m11 = w0 * ry0 * ry0 + w1 * ry1 * ry1 + w2 * ry2 * ry2 + w3 * ry3 * ry3;
            const b0 = w0 * dv0 * rx0 + w1 * dv1 * rx1 + w2 * dv2 * rx2 + w3 * dv3 * rx3;
            const b1 = w0 * dv0 * ry0 + w1 * dv1 * ry1 + w2 * dv2 * ry2 + w3 * dv3 * ry3;
            const det = m00 * m11 - m01 * m01;
            const affineOffset = 4 * i + 2 * component;

            this.particleVel[2 * i + component] = apicV;
            if (Math.abs(det) > 1e-12) {
              this.particleAffine[affineOffset] = (m11 * b0 - m01 * b1) / det;
              this.particleAffine[affineOffset + 1] = (m00 * b1 - m01 * b0) / det;
            } else {
              this.particleAffine[affineOffset] = 0.0;
              this.particleAffine[affineOffset + 1] = 0.0;
            }
          }
        }
      }
  ...
  // them for wall restoration. APIC does not need the pressure delta, but
  // keeping the snapshot is harmless and preserves the shared grid bookkeeping.
  solveIncompressibility(
    numIters: number,
    dt: number,
  ...
  // ── simulate ─────────────────────────────────────────────────────────────
  // Single timestep of the APIC simulation.  The pipeline is:
  //
  ...
  //   7. transferVelocities (G→P) – reconstruct velocity and affine state
  //
  ...
  // the particle simulation.  Particle positions, velocities, affine state,
  // colours, and particleRestDensity are all preserved.
  //
  // What changes:
