export const Utilities = {
    ...
    addVectors: function (out: number[] | Float32Array, a: number[] | Float32Array, b: number[] | Float32Array) {
        out[0] = a[0] + b[0];
        out[1] = a[1] + b[1];
        out[2] = a[2] + b[2];
        return out;
    ...
    subtractVectors: function (out: number[] | Float32Array, a: number[] | Float32Array, b: number[] | Float32Array) {
        out[0] = a[0] - b[0];
        out[1] = a[1] - b[1];
        out[2] = a[2] - b[2];
        return out;
    ...
    dotVectors: function (a: number[] | Float32Array, b: number[] | Float32Array): number {
        return a[0] * b[0] + a[1] * b[1] + a[2] * b[2];
    },

    multiplyVectorByScalar: function (out: number[] | Float32Array, v: number[] | Float32Array, k: number[]) {
        out[0] = v[0] * k[0];
        out[1] = v[1] * k[0];
        out[2] = v[2] * k[0];
        return out;
    ...
    multiplyVectorByNumber: function (out: number[] | Float32Array, v: number[] | Float32Array, k: number) {
        out[0] = v[0] * k;
        out[1] = v[1] * k;
        out[2] = v[2] * k;
        return out;
    ...
    normalizeVector: function (out: number[] | Float32Array, v: number[] | Float32Array) {
    ...
    makePerspectiveMatrix: function (out: Float32Array, fovy: number, aspect: number, near: number, far: number) {
    ...
    premultiplyMatrix: function (out: Float32Array, matrixA: Float32Array, matrixB: Float32Array) { //out = matrixB * matrixA
        const b0 = matrixB[0], b4 = matrixB[4], b8 = matrixB[8], b12 = matrixB[12],
            ...
            a0 = matrixA[0], a1 = matrixA[1], a2 = matrixA[2], a3 = matrixA[3];
        ...
        const a4 = matrixA[4], a5 = matrixA[5], a6 = matrixA[6], a7 = matrixA[7];
        ...
        const a8 = matrixA[8], a9 = matrixA[9], a10 = matrixA[10], a11 = matrixA[11];
        ...
        const a12 = matrixA[12], a13 = matrixA[13], a14 = matrixA[14], a15 = matrixA[15];
    ...
    transformDirectionByMatrix: function (out: number[] | Float32Array, v: number[] | Float32Array, m: Float32Array) {
    ...
    makeLookAtMatrix: function (matrix: Float32Array, eye: number[] | Float32Array, target: number[] | Float32Array, up: number[] | Float32Array) {
        const forwardX = eye[0] - target[0],
            forwardY = eye[1] - target[1],
            forwardZ = eye[2] - target[2];
        const forwardMagnitude = Math.sqrt(forwardX * forwardX + forwardY * forwardY + forwardZ * forwardZ);
        ...
        const rightMagnitude = Math.sqrt(rightX * rightX + rightY * rightY + rightZ * rightZ);
        ...
        const newUpMagnitude = Math.sqrt(newUpX * newUpX + newUpY * newUpY + newUpZ * newUpZ);
