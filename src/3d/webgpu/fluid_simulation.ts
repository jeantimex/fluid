...
export class FluidSimulation {
  ...
  /** Handles particle and wireframe rendering */
  ...
  /**
   * Frustum culling uniform data.
   * Layout: [viewProjection (16 floats), radius, particleCount, padding, padding]
   ...
   * - vertexCount: 6 (two triangles per particle quad)
   * - instanceCount: Reset to 0 before culling, then atomically incremented
  ...
  constructor(
    ...
    // Create renderer (particle billboards + wireframe)
  ...
  /**
   * Dispatches the frustum culling compute pass.
   *
   * This pass tests each particle against the view frustum and builds
   * a compact list of visible particle indices. It also populates the
   * indirect draw buffer with the count of visible particles.
   *
   * Benefits:
   * - No CPU-GPU synchronization needed for particle count
   * - Reduces vertex shader invocations for off-screen particles
   * - Reduces fragment shader work for culled particles
   *
   * @param encoder - Command encoder to record the compute pass to
  ...
  private dispatchCull(
    ...
    // Reset indirect draw buffer (instanceCount = 0)
    ...
    // Pack uniform data
    this.cullParamsData.set(viewProj); // First 16 floats = 4x4 matrix
    this.cullParamsData[16] = config.particleRadius;

    // particleCount is u32, need to use correct view
  ...
  render(viewMatrix: Float32Array): void {
    ...
    // Run frustum culling (populates indirect draw buffer)
    this.dispatchCull(encoder, viewMatrix);

    // Render particles and bounding box
