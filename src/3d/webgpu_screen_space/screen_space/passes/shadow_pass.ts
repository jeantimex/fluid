...
export class ShadowPass {
  ...
  buildShadowVP(frame: ScreenSpaceFrame): Float32Array {
    ...
    const eye = vec3Scale(sunDir, 50);
    const target = { x: 0, y: -2.5, z: 0 }; // Roughly center of the sim
    ...
    const hx = frame.boundsSize.x * 0.5 + 1;
    const hy = frame.boundsSize.y * 0.5 + 1;
    const hz = frame.boundsSize.z * 0.5 + 1;
